...
class AuDrA(pl.LightningModule):
    def __init__(self, args):
        ...
        self.hparams = args
        self.args = args
        self.cur_filename = "test_output_dataframe_ink.csv"
        self.train_storage_dict = {"rating": [], "fname": []} # to save filename and ratings
        self.val_storage_dict = {"rating": [], "fname": []} # to save filename and ratings
        self.test_storage_dict = {"rating": [], "fname": []} # to save filename and ratings
    ...
    def training_step(self, batch, batch_idx):
        images, labels, filenames, ratings  = batch
        self.train_storage_dict['rating'].extend(list(ratings.cpu().numpy()))
        self.train_storage_dict['fname'].extend(list(filenames))
        filelist = np.array(list(filenames))
        ...
        # log = {'train_loss': pred_loss, 'filenames': filelist}
        # log = {'train_loss': pred_loss, 'filenames': filelist}
    ...
    #     ratings = torch.cat([x['train_ratings'] for x in outputs])
    #     preds = torch.cat([x['train_preds'] for x in outputs])
    ...
    #     filenames = np.squeeze(np.array([item for sublist in x['filenames'] for item in sublist]))
    #     # sums = torch.cat([x['log']['train_img_sums'] for x in outputs]) # THIS IS THE ORIGINAL V WITHOUT UNSQUEEZE
    #     # sums = sums.unsqueeze(1) # NEW MODIFICATION TO GET SHAPE TO (1107,1) INSTEAD OF (1107)
    #     df = pd.DataFrame({'ratings': ratings_np, 'predictions': preds_np, 'filenames': filenames})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_storage_dict['rating'].extend(list(ratings.cpu().numpy()))
        self.val_storage_dict['fname'].extend(list(filenames))
        ratings = ratings.float()
    ...
    def validation_epoch_end(self, outputs):
        ...
        preds_np = preds_np.tolist()
        sums_np = sums.cpu()
        sums_np = sums_np.numpy()
        sums_np = sums_np.tolist()
        df = pd.DataFrame({'ratings': ratings_np, 'predictions': preds_np, 'ink':sums_np})
        df.to_csv('validation_output_dataframe_ink.csv')
    ...
    def test_step(self, batch, batch_idx):
        images, labels, filenames, ratings  = batch
        self.test_storage_dict['rating'].extend(list(ratings.cpu().numpy()))
        self.test_storage_dict['fname'].extend(list(filenames))
        # images, labels, ratings = batch
        ratings = ratings.float()
